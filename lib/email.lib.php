...
function common_sendMail($subject, $body, $to, $param = array()) {
	...
	$cMail->SetFrom($param['from']['email'], $param['from']['user']);
	$cMail->AddHeader('Reply-To', $param['from']['email']);
	$cMail->AddHeader('Return-Path', $param['from']['email']);
	$cMail->SetReturnPath($param['from']['email']);

	if (!is_array($to)) $cMail->AddTo($to); //$to = array($to);
	else foreach ($to as $v) {
		switch ($v['type']) {
			default:
				$cMail->AddTo($v['email'], $v['user']);
				break;
			case 'cc':
				$cMail->AddCc($v['email'], $v['user']);
				break;
			case 'bcc':
				$cMail->AddBcc($v['email'], $v['user']);
				break;
		}
	}
	...
	if (!empty($param['attach'])) {
		foreach ($param['attach'] as $v) {
			$cMail->AddAttachLocalFile($v['filename'], $v['mime']);
		}
	}
...
/*
* cPHPezMail Version 1.0 (2004-04-12 03:00 pm +7 GMT)
* COPYRIGHT 2004 CHARIN NAWARITLOHA.
* Contact: charin-nawaritloha@mweb.co.th
*/
...
class EMail
{
	...
	function EMail() {
		...
		$this->nEncoding = 7;
		$this->sTempFileName = SERVER_ROOT.'/shared/attaches/' . md5(time()) . '.tmp';
	...
	{
		if($str_ScreenName)
			$this->sFrom = "=?".$this->sCharset."?B?".base64_encode($str_ScreenName)."?= <$str_Email>";
		else
	...
	function SetReturnPath($int_ReturnPath) {
		$this->sReturnPath = $int_ReturnPath;
	}
	...
	{
		$sTo = str_replace("\r\n", "\n", implode(', ', $this->aTo));
		$sSubject = str_replace("\r\n", "\n", $this->sSubject); // некоторые почтовые сервера плохо относятся к "\r\n"
		$sMessage = str_replace("\r\n", "\n", $this->_MakeMessage());
		$sHeader = str_replace("\r\n", "\n", $this->_MakeHeader());
		$bResponse = mail($sTo, $sSubject, $sMessage, $sHeader, ($this->sReturnPath ? '-f'.$this->sReturnPath : ''));
		return $bResponse;
	}
	...
	function _MakeHeader()
	{
		...
		if($this->sBodyHTML || $this->aPOSTFileAttach) //Check for multipart format
