			...
			<tr class="<?php echo ($i % 2) ? 'odd' : 'even'; ?>">
				<?php
					foreach ($dRaw['model']->fields as $field) {
						...
						echo '<td>';
						if (($field->isListEdit) && ($this->app['user']->checkRoute('articleimage_edit'))) {
							$this->showLink($field->listText($item), 'articleimage_edit', array('id' => $item->id));
						} else  echo $field->listText($item);
						echo '</td>';
					}
				?>
				<td>
                    <?php $this->showLink('&nbsp;X&nbsp;','articleimage_delete', array('id' => $item->id),
