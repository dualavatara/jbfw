...
class ListTemplate extends Template {
	...
	<div class="submenubar">
		<?php $this->showLink('[Список]', 'realty_list')?>
		<?php $this->showLink('[Добавить]', 'realty_add')?>
	</div>
	<table class="list">
		<tr>
			<?php
		...
		<tr class="<?php echo ($i % 2) ? 'odd' : 'even'; ?>">
			...
			foreach ($dRaw['model']->fields as $field) {
				...
				echo '<td>';
				if (($field->isListEdit) && ($this->app['user']->checkRoute('realty_edit'))) {
					$this->showLink($field->listText($item), 'realty_edit', array('id' => $item->id));
				} else  echo $field->listText($item);
				echo '</td>';
			}
			?>
			<td>
				<?php $this->showLink('&nbsp;X&nbsp;', 'realty_delete', array('id' => $item->id), 'onClick="return AdminJS.deleteConfirmation();"');?>
			</td>
		</tr>
		<?php endforeach; ?>
	</table>
