...
class ListTemplate extends Template {
	...
	<div class="submenubar">
        <?php $this->listLink();?>
        <?php $this->showLink('[Добавить]','cartype_add')?>
	</div>
	<table class="list">
		<tr>
			<?php
			...
			<tr class="<?php echo ($i % 2) ? 'odd' : 'even'; ?>">
				<?php
					foreach ($dRaw['model']->fields as $field) {
						...
						echo '<td>';
						if (($field->isListEdit) && ($this->app['user']->checkRoute('cartype_edit'))) {
							$this->showLink($field->listText($item), 'cartype_edit', array('id' => $item->id));
						} else  echo $field->listText($item);
						echo '</td>';
					}
				?>
				<td>
                    <?php $this->showLink('&nbsp;X&nbsp;','cartype_delete', array('id' => $item->id),
